  ...
  return (
    <div className="flex justify-center items-center my-10 lg:my-[70px]">
      <div className="flex flex-col items-center text-white lg:bg-dark lg:py-8 lg:rounded-md">
        {loginError ? (
          <div style={{border: '1px solid red'}} className="text-error text-lg text-center bg-grey py-2 lg:mx-5 w-full lg:max-w-[calc(100%-60px)] rounded mb-2">{loginError}</div>
        ) : null}
        <div className="flex items-center gap-2 text-2xl">
          {error || loginError ? <MdError className="text-error text-[40px]" /> : null} 
          <p>Log In</p>
        </div>
        <form
          onSubmit={handleSubmit}
          className="flex flex-col gap-6 items-center bg-grey px-4 py-6 rounded lg:rounded-none min-w-[300px] lg:min-w-[450px] my-5"
        >
          <div className="flex flex-col items-center">
            ...
            />
            {error && error.includes("email") ? <div className="text-error text-sm text-center mt-1">{error}</div> : null}
          </div>
          <div className="flex flex-col items-center">
            <div className="flex flex-row gap-2 self-start items-center">
              <p>
            ...
            {error && error.includes("password") ? <div className="text-error text-sm text-center">{error}</div> : null}
          </div>
          <button
            type="submit"
            disabled={isLoading}
            className="bg-btn-green py-2 rounded w-full lg:w-[96%] text-white mt-2 mx-4 text-md lg:text-md hover:bg-btn-green-active"
          >
